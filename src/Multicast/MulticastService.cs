...
namespace MeaMod.DNS.Multicast
{
    /// <summary>
    ...
    public class MulticastService : IResolver, IDisposable
    {
        ...
        {
            Debug.WriteLine("Finding network interfaces");
            ...
            try
            {
                ...
                foreach (var nic in knownNics.Where(k => !currentNics.Any(n => k.Id == n.Id)))
                {
                    ...
                    #if DEBUG
                        Debug.WriteLine($"Removed nic '{nic.Name}'.");
                    #endif
...
#if DEBUG
                        Debug.WriteLine($"Found nic '{nic.Name}'.");
#endif
                }
            ...
            {
                Debug.WriteLine("FindNics failed", e);
            }
        }
        ...
        public void OnDnsMessage(object sender, UdpReceiveResult result)
        {
            ...
            {
                Debug.WriteLine("Received malformed message", e);
            ...
            {
                Debug.WriteLine("Receive handler failed", e);
                // eat the exception
            }
        }
